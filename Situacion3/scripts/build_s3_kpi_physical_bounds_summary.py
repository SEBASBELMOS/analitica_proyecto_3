...
def main() -> None:
    ...
    ap.add_argument("--validation-dir", type=Path, required=True)
    ...
    loo = pd.read_csv(args.validation_dir / "loo_spatial_metrics_summary.csv")
    moran = pd.read_csv(maps_dir / "moran_global_by_pollutant_horizon.csv")
    vg_class = pd.read_csv(args.validation_dir / "variogram_residual_structure_classification.csv")
    ...
    for pollutant, min_thr, exc_thr in [("SO2", 6.0, 3.0), ("O3", 12.0, 6.0)]:
        ...
        value = float(row["corrected_rmse"])
        rows.append({
            ...
            "evidence": "LOO-CV covariado con ST-Kriging y postproceso fisico: fallback ConvLSTM si correccion < 0; piso 0 si ConvLSTM tambien < 0.",
        })
    all_row = loo[loo["scope"] == "all"].iloc[0]
    r2 = float(all_row["corrected_r2"])
    rows.append({
        ...
        "evidence": "Calculado sobre SO2+O3 con la misma regla fisica usada para mapas/API; NO2 excluido por n=1 estacion.",
    })
    ...
    rows.append({
        ...
        "evidence": "Moran I recalculado sobre mapas finales fisicamente validos.",
    })
    ...
    rows.append({
        ...
        "value": f"{no_structure}/6 sin estructura en SO2/O3 LOO fisico",
        "status": "cumple_minimo" if no_structure == 6 else "no_cumple",
        "evidence": "Clasificacion distancia-semivarianza sobre residuos LOO corregidos fisicamente; persiste estructura residual.",
    })
    coverage = float(all_row["coverage_95_sigma_kriging"])
    rows.append({
        ...
        "evidence": "Empirico LOO-CV SO2+O3 usando |obs-pred_fisica| <= 1.96*sigma_kriging.",
    })
    degradations = []
    for pollutant in ["SO2", "O3"]:
        ...
        degradations.append(float(r7["corrected_rmse"] / r1["corrected_rmse"] - 1.0))
    ...
    rows.append({
        ...
        "evidence": "Ratio RMSE corregido fisicamente T+7/T+1 sobre SO2 y O3.",
    })
    rows.append({
        ...
        "excellent": "< 3 s",
        "value": np.nan,
        "status": "pendiente_medicion_formal",
        "evidence": "API desplegada; falta medicion formal repetida de latencia end-to-end.",
    })

    kpi = pd.DataFrame(rows)
    out_csv = args.out / "kpi_situacion3_physical_bounds_summary.csv"
    kpi.to_csv(out_csv, index=False)
    summary = {"kpi_csv": str(out_csv), "status_counts": kpi["status"].value_counts().to_dict()}
    (args.out / "summary_kpi_physical_bounds.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
